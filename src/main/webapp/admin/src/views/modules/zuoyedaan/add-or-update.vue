<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="作业答案" prop="zuoyedaan">
          <file-upload
          tip="点击上传作业答案"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.zuoyedaan?ruleForm.zuoyedaan:''"
          @change="zuoyedaanUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.zuoyedaan" label="作业答案" prop="zuoyedaan">
            <el-button type="text" size="small" @click="download(ruleForm.zuoyedaan)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zuoyebianhao = true;
          ...
          if(o=='zuoyedaan'){
            this.ruleForm.zuoyedaan = obj[o];
	    this.ro.zuoyedaan = true;
            continue;
          }
          if(o=='gonghao'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zuoyedaan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuoyedaanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    zuoyedaanUploadChange(fileUrls) {
	this.ruleForm.zuoyedaan = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
