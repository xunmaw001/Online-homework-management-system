<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="作业批阅" prop="zuoyepiyue">
          <file-upload
          tip="点击上传作业批阅"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.zuoyepiyue?ruleForm.zuoyepiyue:''"
          @change="zuoyepiyueUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.zuoyepiyue" label="作业批阅" prop="zuoyepiyue">
            <el-button type="text" size="small" @click="download(ruleForm.zuoyepiyue)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="作业成绩" prop="zuoyechengji">
          <el-input v-model="ruleForm.zuoyechengji" 
              placeholder="作业成绩" clearable  :readonly="ro.zuoyechengji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="作业成绩" prop="zuoyechengji">
              <el-input v-model="ruleForm.zuoyechengji" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="是否及格" prop="shifoujige">
          <el-select v-model="ruleForm.shifoujige" placeholder="请选择是否及格">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="是否及格" prop="shifoujige">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xuesheng = true;
          ...
          if(o=='zuoyepiyue'){
            this.ruleForm.zuoyepiyue = obj[o];
	    this.ro.zuoyepiyue = true;
            continue;
          }
          if(o=='zuoyechengji'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zuoyepiyue/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuoyepiyueCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    zuoyepiyueUploadChange(fileUrls) {
	this.ruleForm.zuoyepiyue = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
